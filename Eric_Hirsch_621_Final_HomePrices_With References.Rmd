...
#### E. Tree based models: Random Forest and Gradient Boosting

Our final models are Random Forest and Gradient Boosting.
...
Our Random Forest model does not improve our score, but Gradient Boosting does.  We land on .12786, which puts us in the 80th percentile.
...
Random Forest (RF) and Gradient Boosting (GB) both rely on decision trees.  Decision trees incorporate sequential choice-point steps, providing different outcomes for each choice-point. Decision trees have the advantage that they do not make assumptions that the dependent variable is linearly related to the independent variables.  In addition, RF and GB both combine multiple trees so the results are averages of many samples, which improves their predictability.  However, the results may be difficut to interpret.
